...
\begin{proof}
  ...
  \end{equation*}
  Then $f(x)$ has two $x$-intercepts at $x=-2,1$, and two vertical asymptotes at $x=-4,3$. By the Intermediate Value Theorem, those four $x$-values are the only places that the function can change the sign of its output. As such, it has been established that $f(x)$ does not change sign over the given intervals. \\
