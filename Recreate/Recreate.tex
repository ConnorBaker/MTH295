...
% \topmargin -0.25in \oddsidemargin -0.25in \evensidemargin -0.25in
% \textheight 9in \textwidth 6.75in \headheight 0in \headsep 0.35in
\usepackage[left=0.905in,right=0.8661in,top=0.80in,bottom=1.25in]{geometry}
\parindent 0in
...
\begin{equation*}
  \int{f(x)dx}, \int_{0}^{\ln(2)}{e^xdx} = e^x \bigg\rvert_{0}^{\ln(2)} = e^{\ln(2)} - e^0 = 2 - 1 = 1.
\end{equation*}
...
\begin{equation*}
  f(x) = |x| = \begin{cases}
   ...
   \end{cases}.
\end{equation*}
Let $a_{ij}= i + j$, for any $i,j\in\N$. If $A$ is a $2\times3$ matrix with $A=[a_{ij}]$, then
\begin{equation*}
  A =
  \begin{bmatrix}
    \ 2 & 3 & 4 \ \ \\
