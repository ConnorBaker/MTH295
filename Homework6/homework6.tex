...
\begin{proof}
  ...
  Consider the function $g:A \to (0,1)$. The function $g$ takes each function in $f$ and creates a list of the outputs of that function as a decimal number in $(0,1)$:
    $$g(f_1) = 0.a_{11} a_{12} a_{13} \dots$$
    $$g(f_2) = 0.a_{21} a_{22} a_{23} \dots$$
    $$g(f_3) = 0.a_{31} a_{32} a_{33} \dots$$
    $$\vdots$$
    $$g(f_n) = 0.a_{n1} a_{n2} a_{n3} \dots$$
    $$\vdots$$
  where $a_{ij}$ is either 0 or 1, the range of $f$. Then $g$ is obviously a one-to-one mapping to $(0,1)$.
...
\begin{proof}
  ...
  Consider the function $g:A \to (0,1)$. The function $g$ takes each function in $f$ and creates a list of the outputs of that function, excluding all trailing zeros (that is, all zeros that are not followed at some point by a nonzero number), then reverses that list (ex: $010010100000\mapsto1010010$), creating a unique binary number in $\N$:
    $$g(f_1) = a_{13} a_{12} a_{11} \dots$$
    $$g(f_2) = a_{23} a_{22} a_{21} \dots$$
    $$g(f_3) = a_{33} a_{32} a_{31} \dots$$
    $$\vdots$$
    $$g(f_n) = a_{n3} a_{n2} a_{n1} \dots$$
    $$\vdots$$
  where $a_{ij}$ is either 0 or 1, the range of $f$. Then $g$ is obviously a one-to-one mapping to $\N$.
