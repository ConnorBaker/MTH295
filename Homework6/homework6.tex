...
\begin{proof}
  We will prove by contradiction. Assume that $|A|=|\N|$. Then, $A=\{f_1,f_2,\dots\}$. Let $g:\N\to\{0,1\}$,
  $$g(n)= \begin{cases}
      0 & \text{if }f_n(n) = 1 \\
      1 & \text{if }f_n(n) = 0
   \end{cases}.$$

   Then, there is some $n$ such that $g(n)\neq f_n(n)$. Since $A$ is the set of all functions $f:\N \to \{0,1\}$, and $g$ is one of those functions and yet not included in the set, our assumption that $|A|=|\N|$ must be incorrect.

  Therefore, by contradiction, $|A|=|\R|$.
\end{proof}
